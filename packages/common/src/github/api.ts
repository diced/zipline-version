...
export class CachedGitHubApi {
  ...
  public async getLatestCommit(): Promise<{ sha: string; pull: boolean } | null> {
    const commits = await cachedFetch<Commits>(this.cache, CACHE_KEYS.latestCommit, `${GITHUB_API_BASE}/commits`, this.githubToken);
  ...
  public async getTagFromSha(sha: string): Promise<Tags[0] | null> {
    const tags = await cachedFetch<Tags>(this.cache, CACHE_KEYS.tagsAll, `${GITHUB_API_BASE}/tags`, this.githubToken);
